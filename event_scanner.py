...
=====================================
עוטף את event_scanner.py ומוסיף ממשק ויזואלי ו-API.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# API — קבלת אירועים מה-DB
# ─────────────────────────────────────────────────────────────────────────────
@app.route("/api/events")
def api_events():
    try:
        events = get_upcoming_events()
        # המר stations מ-JSON string ל-list
        for ev in events:
...
# ─────────────────────────────────────────────────────────────────────────────
# API — שליחת מייל עם PDF
# ─────────────────────────────────────────────────────────────────────────────
@app.route("/api/send-email", methods=["POST"])
def api_send_email():
    data = request.get_json(silent=True) or {}
    to_email = data.get("to", "").strip()
    subject_override = data.get("subject", "").strip()
    ...
    # שינוי זמני של נמען
    original_recipient = CONFIG["recipient_email"]
    ...
    try:
        ...
        CONFIG["recipient_email"] = original_recipient
